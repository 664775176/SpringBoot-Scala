...
protected case class Student(p_id : Int, p_sex : String, p_name: String, p_class_id : Int,
...
object Practice1 {
  ...
  private def statistics(students: List[Student]) = {
    ...
    group.keys.foreach { i =>
      ...
      //语文平均分
      val chineseNums = {
        chineseList.sum / chineseList.size
      }
      println("班级：" + i + " 语文平均分为：" + chineseNums)
      //语文等级比例
      ...
      //数学平均分
      val mathNums = mathList.sum / mathList.size
      println("班级：" + i + " 数学平均分为：" + mathNums)
      //数学等级比例
      ...
      //英语平均分
      val englishNums = englishList.sum / englishList.size
      println("班级：" + i + " 英语平均分为：" + englishNums)
      //英语等级比例
